...
class Vec3Double {
    ...
    update(other) {
        this.x = new JavaDouble(other.x.valueOf());
        this.y = new JavaDouble(other.y.valueOf());
        this.z = new JavaDouble(other.z.valueOf());
        return this;
    }

    distanceTo(other) {
        const dx = this.x.subtract(other.x).valueOf();
        const dy = this.y.subtract(other.y).valueOf();
        const dz = this.z.subtract(other.z).valueOf();
        return new JavaDouble(Math.sqrt(dx * dx + dy * dy + dz * dz));
    }

    clone() {
        return new Vec3Double(this.x.valueOf(), this.y.valueOf(), this.z.valueOf());
    }
}
